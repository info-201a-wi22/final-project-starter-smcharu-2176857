# Reading Files and Mutating Files
...
summary_info$most_patients <- combined_data %>% 
  filter(hospitalizedCumulative == max(hospitalizedCumulative, na.rm = TRUE)) %>% 
  pull(State)

# Value 4: State with most least hospitalized patients
summary_info$least_patients <- combined_data %>% 
  filter(hospitalizedCumulative == min(hospitalizedCumulative, na.rm = TRUE)) %>% 
  pull(State)
