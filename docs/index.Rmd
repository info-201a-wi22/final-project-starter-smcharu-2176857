...
## **CHART 1**:
This chart expresses the amount of allocation funds each state in the 
United States received during the two periods of distribution which happened 
in between January 1, 2020, and June 10, 2020. The reason why we are choosing 
to show this data is because it gives the reader a sense of how the money 
was allocated from state to state. The reader can easily view the amount of 
money that specific states in comparison to under ones. For example, it is 
very clear that New York got more funding than other states and with this, the 
assumtion can be made that there were more cases, or hospilizations in that 
state, if the money was allocated in an accurate manner. 
...
```{r, echo=TRUE}
plot(chart_1)
>>>>>>> 6a0b4a686a88354d0e80cec5d46a93a63928a1f2
```
